...
class Marketplace:
    ...
    def publish(self, producer_id, product):
        self.logger.info("%d is publishing the product %s", producer_id, product)
        ...
        with self.verify_queue_size_lock:
            for lock, products in self.distribuitor[producer_id].items():
                if len(products) >= self.queue_size_per_producer:
                    self.logger.info("queue is full, producer %d should wait", producer_id)
                    return False

                # append is a thread safe method
                self.distribuitor[producer_id][lock].append(product)

        self.logger.info("producer %d has been published the product %s", producer_id, product)
    ...
    def add_to_cart(self, cart_id, product): 
        self.logger.info("consumer is trying to add the product %s into cart %d", product, cart_id)
        ...
        for producer_id, dict in self.distribuitor.items():
            for lock, products in dict.items():
                if product in products:
                    ...
                    self.logger.info("the product %s has been added to cart %d", product, cart_id)

                    return True

        self.logger.error("product %s not available, the consumer should wait", product)
        return False
       

    def remove_from_cart(self, cart_id, product):
        self.logger.info("consumer is trying to remove the product %s from cart %d", product, cart_id)
        ...
        for (producer_id, prod) in self.cart[cart_id]:
            if str(prod) == str(product):
                ...
                self.logger.info("the product %s has been remove from card %d", product, cart_id)

                return True 

        # the product doesn't exists in the cart
        self.logger.error("the product %s doesn't exist in card %d", product, cart_id)
        return False
...
class TestMarketplace(unittest.TestCase):
    ...
    def test_remove_from_cart(self):
        ...
        self.assertTrue(self.marketplace.remove_from_cart(cart_id, products[0]))
        self.assertFalse(self.marketplace.remove_from_cart(cart_id, products[0]))
        self.assertFalse(self.marketplace.remove_from_cart(cart_id, products[1]))
